...
and \pkg{changepoint.geo} \citep{grundy2020high}; Bayesian methods, such as \pkg{bcp} \citep{erdman2008bcp}; and methods that implement online procedures such as \pkg{CPM} \citep{ross2015parametric} and \pkg{FoCUS} \citep{romano2021fast}. The \pkg{changepoints} package \citep{changepoints-package} implements a range of changepoint methods for univariate data, including change in mean and (discontinuous) change in polynomial regression, and multivariate data.
...
\end{itemize}
The \code{cpop} function returns an S4 object for which a number of generic methods, including \code{plot} and \code{summary}, are provided. The default value for \code{sd} is based on a simple estimator that is appropriate for regularly-spaced data. In this case, taking the first difference of \code{y} twice will remove the linear trend within each segment, and the variance of the resulting twice-differenced data is an estimator of 6 times the noise variance.
%
%
...
%
%
The function \code{residuals(object)}  creates a single column matrix containing the residuals.
%\newpage
%\subsubsection{example}
\begin{CodeChunk}
\begin{CodeInput}

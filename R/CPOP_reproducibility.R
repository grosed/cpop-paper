...
#For outputting pdf versions of figures (code by Rob Hyndman)
...
###implement three methods -- cpop, difference and treat as change-in-mean, and trend-filtering
...
savepdf("figures/Change-in-slope_examples",width=10,height=8)
par(mfrow=c(2,2))
...
p <- p + theme_bw()
savepdf("figures/simulate-example",width=8,height=4.5)
print(p)
dev.off()
...
savepdf("figures/cpop-example1",width=8,height=4.5)
print(p)
dev.off()
...
savepdf("figures/cpop-example-uneven",width=8,height=4.5)
print(p)
dev.off()
...
changepoints <- c(0, 25, 50, 100)
change.slope <- c(0.2, -0.3, 0.2, -0.1)
x <- 1:200 ###x-values now unevenly spaced
sd <- x/100
y <- simulate(x, changepoints, change.slope, sd) #simulate data
mu <- simulate(x, changepoints, change.slope, sd = 0)
...
savepdf("figures/cpop-example-uneven1",width=6,height=6)
print(p)
dev.off()
savepdf("figures/cpop-example-uneven2",width=6,height=6)
print(p.true)
dev.off()
...
##FIGURE 6
savepdf("figures/cpop-CPU",width=8,height=4.5)
...
###FIGURE 7
savepdf("figures/cpop-minseg1",width=6,height=6)
p <- plot(res)
p <- p + geom_vline(xintercept = changepoints[-1], color = "blue", linetype = "dashed")
p <- p + geom_line(aes(y = mu), color = "blue", linetype = "dashed")
print(p)
...
savepdf("figures/cpop-minseg2",width=6,height=6)
p <- plot(res.min)
p <- p + geom_vline(xintercept = changepoints[-1], color = "blue", linetype = "dashed")
p <- p + geom_line(aes(y = mu), color = "blue", linetype = "dashed")
print(p)
dev.off()
savepdf("figures/cpop-minseg3",width=6,height=6)
p <- plot(res.min30)
p <- p + geom_vline(xintercept = changepoints[-1], color = "blue", linetype = "dashed")
p <- p + geom_line(aes(y = mu), color = "blue", linetype = "dashed")
print(p)
dev.off()
savepdf("figures/cpop-minseg4",width=6,height=6)
p <- plot(res.min40)
p <- p + geom_vline(xintercept = changepoints[-1], color = "blue", linetype = "dashed")
p <- p + geom_line(aes(y = mu), color = "blue", linetype = "dashed")
print(p)
...
savepdf("figures/cpop-cropsA1",width=6,height=6)

plot(res.crops)
dev.off()

savepdf("figures/cpop-cropsA2",width=6,height=6)
i <- which.min(BIC)
p <- plot(models[[i]])
p <- p + geom_vline(xintercept = changepoints[-1], color = "blue", linetype = "dashed")
p <- p + geom_line(aes(y = mu), color = "blue", linetype = "dashed")
...
savepdf("figures/cpop-cropsB1",width=6,height=6)
plot(res.crops)
dev.off()
savepdf("figures/cpop-cropsB2",width=6,height=6)
#"elbow" plot
plot(segs[,"m"],segs[,"Qm"],lwd=2,type="l",xlab="No. of changepoints",ylab="Unpenalised Cost")
abline(v = 10,col = 2)
dev.off()

savepdf("figures/cpop-cropsB3",width=12,height=6)
models <- cpop.crops.models(res.crops)
p <- plot(models[[5]])
...
savepdf("figures/cpop-data1",width=6,height=6)
par(mfrow=c(1,1))
...
dev.off()
##initial estimate
savepdf("figures/cpop-data2",width=6,height=6)
plot(res) ##can we change plot so that we can label axes such as this?
dev.off()

## second estimate
savepdf("figures/cpop-data3",width=6,height=6)
plot(res2)
dev.off()
...
savepdf("figures/cpop-data4",width=6,height=6)
par(mfrow=c(1,1))
...
for(j in 1:4) lines(exp(x),exp(y.est[j,]),lty=2,lwd=2,col=j)
dev.off()
